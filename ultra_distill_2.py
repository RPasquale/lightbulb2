...
class RLHFDataset(Dataset):
    def __init__(self, rlhf_data: List[Dict], tokenizer, max_length: int = 512, max_attention_length: int = 10):
        self.data = rlhf_data
        self.tokenizer = tokenizer
        self.max_length = max_length
    ...
    def __getitem__(self, idx):
        ...
        attention_weights = entry.get('attention_weights', [])
        ...
        # Pad or truncate attention_weights to max_attention_length
        if len(attention_weights) < self.max_attention_length:
            padding_length = self.max_attention_length - len(attention_weights)
            attention_weights = attention_weights + [0.0] * padding_length
        else:
            attention_weights = attention_weights[:self.max_attention_length]
...
def custom_collate_fn(batch):
    ...
    correct_option = [item['correct_option'] for item in batch]
    incorrect_option = [item['incorrect_option'] for item in batch]
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in tqdm(train_loader_form2, desc="Form 2 Training"):
            ...
            # Get attention weights from batch
            attention_weights_batch = batch['attention_weights'].to(device)  # Shape: (batch_size, max_attention_length)
            ...
            weighted_ppo_loss = (ppo_loss * attention_weights_batch.mean(dim=1)).mean()
            # Backpropagation
            ppo_agent.optimizer.zero_grad()
            if scaler:
                scaler.scale(weighted_ppo_loss).backward()
